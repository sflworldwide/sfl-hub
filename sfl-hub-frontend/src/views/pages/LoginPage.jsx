...
import {
  PersonOutline as FaUser,
  LockOutlined as FaLock,
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${api.BackendURL}/users/${encodedUrl}`,
        encryptedData,
        {
      ...
      if (res.status === 200 && res.data?.user?.data) {
        const decryptedName = CryptoJS.AES.decrypt(
          res.data.user.data.p_name,
          SECRET_KEY
        ).toString(CryptoJS.enc.Utf8);
        const decryptedEmail = CryptoJS.AES.decrypt(
          res.data.user.data.p_email,
          SECRET_KEY
        ).toString(CryptoJS.enc.Utf8);
        const decryptedPhone = CryptoJS.AES.decrypt(
          res.data.user.data.p_phonenum,
          SECRET_KEY
        ).toString(CryptoJS.enc.Utf8);
        const decryptedUsername = CryptoJS.AES.decrypt(
          res.data.user.data.p_username,
          SECRET_KEY
        ).toString(CryptoJS.enc.Utf8);
        const decryptedPersonID = CryptoJS.AES.decrypt(
          res.data.user.data.p_personID,
          SECRET_KEY
        ).toString(CryptoJS.enc.Utf8);
        const decryptedOldPersonID = CryptoJS.AES.decrypt(
          res.data.user.data.p_OldPersonID,
          SECRET_KEY
        ).toString(CryptoJS.enc.Utf8);
        const encryptedAccountNumber = res.data.user.data.p_account_number;
        const decryptedaccount_number = encryptedAccountNumber
          ? CryptoJS.AES.decrypt(encryptedAccountNumber, SECRET_KEY).toString(
              CryptoJS.enc.Utf8
            )
          : null;
