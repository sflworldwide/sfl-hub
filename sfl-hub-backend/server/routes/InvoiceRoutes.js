...
import { generateInvoiceGetRateController } from '../controllers/InvoiceController.js';
...
const conditionalMiddleware = (middleware) => {
  return (req, res, next) => {
    if (process.env.NODE_ENV === 'middleware') {
      ...
      return next(); // skip middleware if not in 'middleware' mode
    }
  };
};


//uter.post("/generateInvoiceGetRate",authenticateUser, generateInvoiceGetRateController);
router.post("/generateInvoiceGetRate", conditionalMiddleware(authenticateUser), generateInvoiceGetRateController);
...
if (decryptedUrl === '/generateInvoice/generateInvoiceGetRate') {
  return runWithOptionalMiddleware(authenticateUser, generateInvoiceGetRateController);
}
