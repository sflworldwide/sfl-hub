...
// Define StyledTextField and StyledTableTextField
...
// Define ResponsivePackageTable
const ResponsivePackageTable = ({
  packageDetails,
  formErrors,
  handlePackageRowChange,
  handleDeleteRow,
  isEnvelope,
  // Changed from 'is' to 'isEnvelope'
  weightUnit,
  dimensionUnit,
  chargeableUnit,
...
const GetRate = ({ setActiveModule, setActiveTab }) => {
  ...
  // Fetch city list for fromCountry when Giszip === 1
  ...
  // Fetch city list for toCountry when Gresiszip === 1
  ...
  useEffect(() => {
    ...
    setPackageDetails(prevRows => {
      const updatedRows = prevRows.map(row => {
        const newChargeableWeight = calculateChargeableWeight(row, fromDetails.fromCountry, toDetails.toCountry);
        if (row.chargeableWeight !== newChargeableWeight) {
          return { ...row, chargeableWeight: newChargeableWeight };
        }
        return row;
      });
      if (updatedRows.some((row, idx) => row.chargeableWeight !== prevRows[idx].chargeableWeight)) {
        return updatedRows;
      }
      return prevRows;
    });
  }, [fromDetails.fromCountry, toDetails.toCountry, packageDetails, isEnvelope]);
  ...
  const handlePackageRowChange = (index, field, value) => {
    ...
    if (['packageNumber', 'weight', 'length', 'width', 'height'].includes(field) && !/^\d*\.?\d*$/.test(value)) return;
    setPackageDetails((prevRows) => {
  ...
  const handleReset = () => {
    ...
    updateToDetails({
      ...
      residential: 'No', // Ensure default is 'No'
      packageType: 'Package',
    });
    setPackageDetails([
      {
        ...
        <CardContent sx={{ padding: '16px' }}>
            ...
            <Box>
              <FormControl fullWidth>
                <Autocomplete
                  ...
                  getOptionLabel={(option) => option.label}
                  value={
                    toDetails.residential
                      ? { value: toDetails.residential, label: toDetails.residential }
                      : { value: 'No', label: 'No' } // Default to 'No'
                  }
                  onChange={(event, newValue) => updateToDetails({ residential: newValue?.value || 'No' })}
                  renderInput={(params) => (
                    <StyledTextField
                      {...params}
                      className="small-textfield"
          ...
          <Box sx={{ marginBottom: '16px' }}>
            ...
            <Button
              ...
              sx={{
                marginTop: '8px',
                backgroundColor: isEnvelope ? '#e0e0e0' : '#1976d2', // Changed to a standard MUI blue
                color: 'white',
                textTransform: 'uppercase',
